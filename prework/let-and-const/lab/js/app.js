...
function displayPics() {
  ...
  // there was an error because const is locally scoped in the while loop block and then we tried to console.log it outside the while loop.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for (let i = 0; i < names.length; i++) {
    if (event.target.id === allProducts[i].name) {
      ...
      console.log(
        `${event.target.id} has
          ${allProducts[i].votes}votes in
          ${allProducts[i].views} views`
      );
    }
  }
